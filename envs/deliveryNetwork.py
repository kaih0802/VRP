...
class DeliveryNetwork():
    def __init__(self, settings, delivery, distance, data_csv=None,):
    ...
    def render_tour(self, remaining_deliveries, VRP_solution):
        ...
        for k in range(self.n_vehicles):
            ...
            for i in range(1, len(VRP_solution[k]) - 1):
                ...
                print(
                    f"node: {delivery['id']}  arrival time: {tour_time:.2f}  [ {delivery['time_window_min']}-{delivery['time_window_max']} ] ")
                tour_time = tour_time_after_waiting
            print(f"** **")

        fig = plt.figure()
        # PRINT DELIVERY
        plt.scatter(45.062786, 7.678686, c='green', marker='s')
        for _, ele in self.delivery_info.items():
    ...
    def _add_tour(self, VRP_solution):
        ...
        for k in range(self.n_vehicles):
            if len(VRP_solution[k]) == 0:
                continue
            plt.plot(
                [45.062786, self.delivery_info[VRP_solution[k][1]]['lat']],
                [7.678686, self.delivery_info[VRP_solution[k][1]]['lng']],
            ...
            plt.plot(
                [self.delivery_info[VRP_solution[k][-2]]['lat'], 45.062786],
                [self.delivery_info[VRP_solution[k][-2]]['lng'], 7.678686],
                color=dict_vehicle_char[k][0]
            )